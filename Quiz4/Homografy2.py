...
def warp(dst_img, src_img, M, blend):
    ...
    warp_dst = cv.warpPerspective(dst_img, M, (dst_h + src_h , dst_w*2))     
    ...
    #             if 'averaging':
    #                 # print('averaging')
    #                 warp_dst[x,y,:] = (warp_img[x,y,:] + src_img[x,y,:])/2
    #             elif 'feathering':
    #                 warp_dst[x,y,:] = (warp_img[y,x,:]*w2 + src_img[x,y,:]*w1)/(w1 + w1)
    #         else:
    #             warp_dst[x,y,:] = src_img[x,y,:]
...
while len(files) != 0:
    for j, dst_file in enumerate(files):
        ...
        if len(good)>maxpoint:
            ...
            src_img = warp(dst_img, src_img, M, 0)
