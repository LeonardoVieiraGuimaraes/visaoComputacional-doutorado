...
for cont, especie in enumerate(['Italia', 'RedGlobe','Vitoria']):
    for temp, tempo in enumerate(range(0,25,6)):
        for i in range(1,31):
            ...
            (h1, _) = np.histogram(gray.ravel(), 256,[0,256], density = True)         
            cx[n] = h1
            cy[n] = [cont, temp]
            
            try:
                os.makedirs('C:/Users/Leonardo/Google Drive/Imagens uva/Fotos/Hist' + past + 'G/')
            ...
            plt.savefig('C:/Users/Leonardo/Google Drive/Imagens uva/Fotos/Hist' + past + 'G/' + especie + '_' + str(tempo) + '_' + str(i) + '.jpg')
            plt.clf()
            n = n+1
...
for cv in [3, 5, 6]:
    ...
    for treino in range(0,cv):            
        ...
        np.savetxt(ptr + 'Xtreino' + past + 'G_' + str(cv) + '.txt', np.array(Xtreino[treino]), delimiter = " ")
        np.savetxt(ptr + 'Ytreino' + past + 'G_' + str(cv) + '.txt', np.array(Ytreino[treino]), delimiter = " ")
        np.savetxt(pte + 'Xteste' + past + 'G_' + str(cv) + '.txt', np.array(Xteste[treino]), delimiter = " ")
        np.savetxt(pte + 'Yteste' + past + 'G_' + str(cv) + '.txt', np.array(Yteste[treino]), delimiter = " ")
