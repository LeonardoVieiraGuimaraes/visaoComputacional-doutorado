...
for especie in ['Verde', 'Vermelha']:

    for tempo in range(1,3):

        tempo = str(tempo)

        imgrgb = cv2.imread('C:/Users/Leonardo/Google Drive/Imagens uva/Fotos/Fotos/' + especie + '/' + tempo + '.jpg')

        filtro = cv2.GaussianBlur(imgrgb,(11,11),0)

        imgLab = cv2.cvtColor(filtro,cv2.COLOR_BGR2Lab)

        #imgLab = retirad(imgLab,0,1,0)
        ...
        pasta = "Fotos/Fotos/Processo/" + especie
        try:
            os.makedirs(pasta)
